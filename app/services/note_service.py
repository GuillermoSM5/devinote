...
class NoteService:
    ...
    def user_can_read(self, user_id: int, note: Note) -> bool:
        ...
        label_ids = self.label.list_label_ids_for_note(note_id=note.id)
    ...
    def list_visible(self, user_id: int) -> list[Note]:
        owned = self.notes.list_owned(owner_id=user_id)

        direct_id = self.shares.list_note_ids_shared_directly(user_id)

        shared_label_ids = self.shares.list_label_ids_shared_with_user(user_id)

        ids_by_label = self.label.list_note_ids_by_label_ids(shared_label_ids)

        combined_ids = list({*direct_id, *ids_by_label})
        shared = self.notes.list_by_ids(ids=combined_ids)

        combined = {
            note.id: note for note in owned
        }
    ...
    def create(self, owner_id: int, payload: NoteCreate) -> Note:
        ...
        if payload.lable_ids:
            self._set_labels(owner_id, note.id, payload.lable_ids)
        return note
    ...
    def update(self, user_id: int, note_id: int, payload: NoteUpdate) -> Note:
        ...
        if not self.user_can_edit(user_id, note):
            raise HTTPException(
                status_code=403, detail="Usuario no Autorizado")
        ...
        if label_ids is not None:
            if note.owner_id != user_id:
                raise HTTPException(
                    status_code=404, detail="No existe o no autorizado")
    ...
    def _set_labels(self, owner_id: int, note_id: int, label_ids: list[int]) -> None:
        valid_ids = self.label.list_ids_for_owner_subset(
