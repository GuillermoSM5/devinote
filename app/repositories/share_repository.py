...
class ShareRepository:
    ...
    def upsert_note_share(self, note_id: int, user_id: int, role: str) -> NoteShare:
        ...
        share_db = NoteShare(note_id=note_id, user_id=user_id, role=role)
        self.db.add(share_db)
        self.db.commit()
        self.db.refresh(share_db)
        return share_db

    def remove_not_share(self, note_id: int, user_id: int) -> None:
    ...
    def upsert_label_share(self, label_id: int, user_id: int, role: str) -> LabelShare:
        ...
        share_db = LabelShare(label_id=label_id, user_id=user_id, role=role)
        self.db.add(share_db)
        self.db.commit()
        self.db.refresh(share_db)
        return share_db
    ...
    def has_any_label_share(self, label_ids: list[int], user_id: int, role: str | None = None) -> bool:
        ...
        label_col = col(LabelShare.label_id)

        query = select(LabelShare).where(
            label_col.in_(label_ids),
